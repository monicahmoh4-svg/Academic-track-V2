...
function PT({ children, sub }) {
  return (
    <div style={{ marginBottom: '1.6rem' }}>
      <h1 style={{ fontFamily:'var(--font-h)', fontSize:'clamp(1.4rem,4vw,1.7rem)', color:'var(--text)', fontWeight:600 }}>{children}</h1>
...
export default function AdminDashboard() {
  ...
  const load = useCallback(async (sec) => {
    const ep = { stats:'/admin/stats', students:'/admin/students', subs:'/admin/submissions', notifs:'/admin/notifications', msgs:'/admin/messages', mods:'/admin/moderators' };
    try {
      if (ep[sec]) { const r = await api.get(ep[sec]); setData(d => ({ ...d, [sec]: r.data })); }
    } catch (e) { showToast(e.response?.data?.error||'Failed to load.','error'); }
  }, []);

  useEffect(() => { load('stats'); load('students'); }, [load]);

  const sw = (p) => { setPanel(p); load(p); if (p==='dash') load('stats'); setSidebar(false); };
  ...
  return (
    <div className="admin-shell">
      ...
      <aside className={`admin-sidebar${sidebarOpen?' open':''}`}>
        {/* Brand */}
        <div style={{ padding:'1.2rem 1.3rem 1rem', borderBottom:'1px solid rgba(255,255,255,.12)', display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:36,height:36,borderRadius:9,background:'rgba(255,255,255,.2)',border:'1px solid rgba(255,255,255,.3)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:19,flexShrink:0 }}>🛡️</div>
          <div>
            <div style={{ fontFamily:'var(--font-h)',fontSize:'1rem',color:'#fff',fontWeight:700,lineHeight:1.1 }}>AcademiTrack</div>
            <div style={{ fontSize:'.59rem',color:'#fbbf24',letterSpacing:'1.5px',textTransform:'uppercase',fontWeight:700,marginTop:2 }}>Admin Portal</div>
          </div>
          {/* Close button — mobile only */}
          <button onClick={() => setSidebar(false)}
            style={{ marginLeft:'auto',background:'rgba(255,255,255,.15)',border:'none',color:'#fff',borderRadius:6,width:28,height:28,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,flexShrink:0 }}
            className="admin-hamburger">✕</button>
        </div>

        {/* Nav */}
        <nav style={{ flex:1,padding:'.7rem 0',overflowY:'auto' }}>
          {groups.map(g => (
            <div key={g}>
        ...
        {/* User + logout */}
        <div style={{ padding:'.9rem 1.2rem',borderTop:'1px solid rgba(255,255,255,.12)' }}>
          <div style={{ background:'rgba(255,255,255,.12)',border:'1px solid rgba(255,255,255,.18)',borderRadius:8,padding:'.65rem .85rem',marginBottom:'.65rem' }}>
            <div style={{ fontSize:'.79rem',fontWeight:600,color:'#fff',marginBottom:'.1rem',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{admin?.name}</div>
            <div style={{ fontSize:'.69rem',color:'rgba(255,255,255,.6)' }}>{admin?.role}</div>
          </div>
          <button onClick={logout}
            style={{ width:'100%',padding:'.42rem',borderRadius:7,background:'rgba(255,255,255,.12)',border:'1px solid rgba(255,255,255,.25)',color:'#fff',fontFamily:'var(--font-b)',fontSize:'.82rem',fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:6 }}>
            ↩ Logout
          </button>
        </div>
      </aside>
      ...
      {/* Mobile overlay */}
      <div className={`admin-overlay${sidebarOpen?' open':''}`} onClick={()=>setSidebar(false)} />
        ...
        <div className="admin-topbar">
          <div style={{ display:'flex',alignItems:'center',gap:10 }}>
            ...
            <button className="admin-hamburger"
              ...
              style={{ background:'none',border:'1.5px solid var(--border)',borderRadius:'var(--r)',padding:'6px 8px',cursor:'pointer',display:'flex',flexDirection:'column',gap:4,flexShrink:0 }}>
              {[0,1,2].map(i=><span key={i} style={{ display:'block',width:17,height:2,background:'var(--text3)',borderRadius:2 }}/>)}
            </button>
            <div style={{ width:3,height:20,background:'linear-gradient(135deg,var(--primary),var(--teal))',borderRadius:2,flexShrink:0 }}/>
            <span style={{ fontFamily:'var(--font-h)',fontSize:'clamp(1rem,3vw,1.15rem)',color:'var(--text)',fontWeight:600,whiteSpace:'nowrap' }}>
              {NAV.find(n=>n.id===panel)?.ico} {NAV.find(n=>n.id===panel)?.lbl||'Dashboard'}
            </span>
          </div>
          <div style={{ display:'flex',alignItems:'center',gap:8,flexShrink:0 }}>
            <div style={{ width:7,height:7,borderRadius:'50%',background:'#22c55e',flexShrink:0 }}/>
            <span style={{ fontSize:'.8rem',color:'var(--text3)',display:'none' }} className="topbar-user">
              <strong style={{ color:'var(--primary)' }}>{admin?.username}</strong>
            </span>
          </div>
        </div>
      ...
      <style>{`
        @media (min-width: 768px) { .topbar-user { display: inline !important; } }
      `}</style>
    </div>
  );
}
...
function DashPanel({ stats, onNav }) {
  const sc = [
    { lbl:'Students',    val:stats.students      ||0, c:'#0891b2', ico:'👥' },
    { lbl:'Submissions', val:stats.submissions   ||0, c:'#1a56db', ico:'📤' },
    { lbl:'Approved',    val:stats.approved      ||0, c:'#059669', ico:'✅' },
    { lbl:'Pending',     val:stats.pendingReview ||0, c:'#d97706', ico:'⏳' },
    { lbl:'Messages',    val:stats.pendingMessages||0,c:'#dc2626', ico:'✉️' },
  ];
  return (
    <div>
      <PT sub="System overview and quick actions">Dashboard</PT>
      <div className="sgrid">
        {sc.map(s=>(
          <div key={s.lbl} className="scard">
            <div style={{ fontSize:'1.3rem',marginBottom:'.2rem' }}>{s.ico}</div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:'1.2rem' }}>
        ...
        <div className="card">
          ...
          <table style={{ width:'100%',fontSize:'.86rem',borderCollapse:'collapse' }}>
            <tbody>
              <tr><td style={{ color:'var(--text3)',padding:'.38rem 0',width:'55%' }}>Approval Rate</td><td style={{ fontWeight:600,color:'var(--green)' }}>{stats.submissions?Math.round((stats.approved/stats.submissions)*100):0}%</td></tr>
              <tr><td style={{ color:'var(--text3)',padding:'.38rem 0' }}>Pending Review</td><td style={{ fontWeight:600,color:'#d97706' }}>{stats.pendingReview||0}</td></tr>
              <tr><td style={{ color:'var(--text3)',padding:'.38rem 0' }}>Status</td><td><span className="badge b-green">● Online</span></td></tr>
            </tbody>
          </table>
        </div>
      </div>
...
function StudentsPanel({ students, onAdd, onRefresh }) {
  const del = async (id) => {
    ...
    try { await api.delete(`/admin/students/${id}`); showToast('Removed.','success'); onRefresh(); }
  ...
  return (
    <div>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1.4rem',gap:'1rem',flexWrap:'wrap' }}>
        <PT sub="Students assigned to you">Students</PT>
        <button className="btn btn-primary btn-sm" onClick={onAdd}>+ Add Student</button>
      </div>
      <div className="card" style={{ padding:0 }}>
        <div className="tbl-wrap">
          <table className="dtbl">
            <thead><tr><th>Name</th><th>Reg No.</th><th>Phone</th><th>Level</th><th>Dept</th><th>Status</th><th></th></tr></thead>
            <tbody>
              {students.length===0 && <tr><td colSpan={7} style={{ textAlign:'center',color:'var(--text3)',padding:'2rem' }}>No students yet.</td></tr>}
              {students.map(s=>(
                <tr key={s.id}>
                  <td style={{ fontWeight:500,whiteSpace:'nowrap' }}>{s.first_name} {s.last_name}</td>
...
function SubsPanel({ subs, onReview }) {
  return (
    <div>
      <PT sub="Review and score student submissions">All Submissions</PT>
      <div className="card" style={{ padding:0 }}>
        <div className="tbl-wrap">
          <table className="dtbl">
            <thead><tr><th>Title</th><th>Student</th><th>Type</th><th>Level</th><th>Status</th><th>Score</th><th></th></tr></thead>
            <tbody>
              {subs.length===0 && <tr><td colSpan={7} style={{ textAlign:'center',color:'var(--text3)',padding:'2rem' }}>No submissions yet.</td></tr>}
              {subs.map(s=>(
                <tr key={s.id}>
                  <td style={{ maxWidth:160,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }} title={s.title}>{s.title}</td>
                  <td className="tsm" style={{ whiteSpace:'nowrap' }}>{s.first_name} {s.last_name}</td>
...
function NotifsPanel({ notifs, students, onRefresh }) {
  ...
  return (
    <div>
      <PT sub="Send announcements to students">Notifications</PT>
      <div className="card">
        ...
        <form onSubmit={send}>
          ...
          {f.recipientType==='student'&&<div className="fg"><label>Select Student</label><select value={f.recipientId} onChange={e=>setF(p=>({...p,recipientId:e.target.value}))}><option value="">— Choose —</option>{students.map(s=><option key={s.id} value={s.id}>{s.first_name} {s.last_name} ({s.reg_number})</option>)}</select></div>}
      ...
      <div className="card">
        ...
        {notifs.length===0?<p className="muted tsm">No notifications sent yet.</p>:notifs.map(n=>(
          <div key={n.id} style={{ background:'var(--surface2)',border:'1px solid var(--border)',borderRadius:'var(--r)',padding:'.9rem',marginBottom:'.6rem' }}>
            <div style={{ display:'flex',justifyContent:'space-between',marginBottom:'.35rem',flexWrap:'wrap',gap:'.4rem' }}>
              <strong style={{ fontSize:'.9rem' }}>{n.title}</strong>
              <span className="tsm muted">{new Date(n.created_at).toLocaleDateString()}</span>
            </div>
...
function MsgsPanel({ msgs, onRefresh }) {
  ...
  const send=async(id)=>{
    const reply=replies[id];if(!reply?.trim())return;
  ...
  return (
    <div>
      ...
      {msgs.length===0?<div className="card"><p className="muted">No messages yet.</p></div>:msgs.map(m=>(
        <div key={m.id} className="card">
          <div style={{ display:'flex',justifyContent:'space-between',marginBottom:'.5rem',flexWrap:'wrap',gap:'.4rem' }}>
            <strong style={{ fontSize:'.92rem' }}>{m.subject}</strong>
            <span className="tsm muted">{m.first_name} {m.last_name} · {new Date(m.created_at).toLocaleDateString()}</span>
          </div>
          <div className="tsm" style={{ color:'var(--text)',lineHeight:1.65,marginBottom:'.75rem' }}>{m.body}</div>
          {m.reply?(
            <div style={{ background:'#e0f7fa',borderRadius:'var(--r)',padding:'.7rem',borderLeft:'3px solid #0891b2' }}>
              <div style={{ fontSize:'.72rem',fontWeight:700,color:'#0891b2',marginBottom:'.2rem' }}>Your reply:</div>
              <div className="tsm">{m.reply}</div>
            </div>
          ):(
            <div style={{ display:'flex',gap:'.5rem' }}>
              <input type="text" placeholder="Type reply…" value={replies[m.id]||''} onChange={e=>setReplies(p=>({...p,[m.id]:e.target.value}))}
                style={{ flex:1,minWidth:0,background:'var(--surface)',border:'1.5px solid var(--border2)',borderRadius:'var(--r)',padding:'.52rem .85rem',color:'var(--text)',fontFamily:'var(--font-b)',fontSize:'.86rem',outline:'none' }}/>
              <button className="btn btn-primary btn-sm" onClick={()=>send(m.id)} disabled={loading[m.id]}>{loading[m.id]?<span className="spin"/>:'Reply'}</button>
            </div>
          )}
        </div>
      ))}
  ...
  return (
    <div>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1.4rem',flexWrap:'wrap',gap:'1rem' }}>
        <PT sub="Manage moderator accounts">Moderators</PT>
      ...
      <div className="modal">
        ...
        <form onSubmit={submit}>
          <div className="fgrid">
            ...
            <div className="fg full"><label>Reg Number *</label><input type="text" placeholder="REG/2024/001" value={f.regNumber} onChange={set('regNumber')}/></div>
            <div className="fg full"><label>Phone *</label><input type="tel" placeholder="+254 700 000 000" value={f.phone} onChange={set('phone')}/></div>
            <div className="fg full"><label>Email</label><input type="email" placeholder="student@uni.edu" value={f.email} onChange={set('email')}/></div>
            <div className="fg full"><label>Academic Level</label><select value={f.academicLevel} onChange={set('academicLevel')}><option>Bachelor's Degree</option><option>Master's Degree</option><option>PhD / Postgraduate</option></select></div>
    ...
    <div className="moverlay open">
      <div className="modal">
        ...
        <form onSubmit={submit}>
          ...
          <div className="fg"><label>Email *</label><input type="email" placeholder="mod@uni.edu" value={f.email} onChange={set('email')}/></div>
          ...
          <div className="fg"><label>Role</label><select value={f.role} onChange={set('role')}><option>Moderator</option><option>Senior Moderator</option><option>Super Admin</option></select></div>
          {err&&<div className="err-box">{err}</div>}
...
function ReviewModal({ sub, onClose, onDone }) {
  ...
  const act=async(action)=>{
    ...
    try{
      ...
      showToast(action==='approve'?'Approved!':action==='reject'?'Rejected.':'Advanced!',action==='reject'?'error':'success');
      onDone();
    }catch(e){showToast(e.response?.data?.error||'Failed.','error');}
    finally{setL(false);}
  };
    ...
    <div className="moverlay open">
      <div className="modal">
        ...
        <div style={{ marginBottom:'1rem' }}>
          <strong style={{ fontSize:'.95rem' }}>{sub.title}</strong>
          <div className="tsm muted mt1">By: {sub.first_name} {sub.last_name} ({sub.reg_number})</div>
        ...
        <div style={{ background:'var(--surface2)',borderRadius:'var(--r)',padding:'.9rem',marginBottom:'1rem',maxHeight:160,overflowY:'auto',border:'1px solid var(--border)',fontSize:'.87rem',lineHeight:1.65 }}>{sub.content}</div>
        {sub.ai_feedback&&(
          <div style={{ background:'#fef9c3',border:'1px solid #fde047',borderRadius:'var(--r)',padding:'.75rem',marginBottom:'.9rem' }}>
            <div style={{ fontSize:'.71rem',fontWeight:700,color:'#d97706',marginBottom:'.2rem',textTransform:'uppercase',letterSpacing:'1px' }}>🤖 AI Feedback</div>
            <div className="tsm" style={{ color:'var(--text2)' }}>{sub.ai_feedback}</div>
          </div>
        )}
        {hist.length>0&&(
          <div style={{ marginBottom:'.9rem' }}>
            <div style={{ fontSize:'.7rem',fontWeight:700,color:'var(--text3)',marginBottom:'.35rem',textTransform:'uppercase',letterSpacing:'.5px' }}>History</div>
        ...
        <div className="fg"><label>Manual Score (0–100)</label><input type="number" min="0" max="100" placeholder="Leave blank to keep AI score" value={score} onChange={e=>setScore(e.target.value)}/></div>
        <div className="fg"><label>Advance to Level</label><select value={advLvl} onChange={e=>setAdvLvl(e.target.value)}>{LEVELS.map(l=><option key={l}>{l}</option>)}</select></div>
        <div className="fg"><label>Moderator Notes</label><textarea rows={3} placeholder="Feedback for student…" value={notes} onChange={e=>setNotes(e.target.value)}/></div>
        <div style={{ display:'flex',gap:'.5rem',flexWrap:'wrap' }}>
          ...
          <button className="btn btn-amber"   disabled={loading} onClick={()=>act('advance')}>{loading?<span className="spin"/>:'→ Advance'}</button>
